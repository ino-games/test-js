...
function call(lines: any[]) {
  ...
  for (let i = 0; i < 5; i++) {
    const line = lines.slice(i * 5, i * 5 + 5);
    const pay = checkPayLine(line, paySymbols, nonPaySymbols);
    if (pay) {
      results.push([pay, i * 5 + pay[1]]);
    }
  }
...
function checkPayLine(line: any[], paySymbols: any[], nonPaySymbols: number[]) {
