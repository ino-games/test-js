function call(lines: number[]): WinningCombinationsResult {
  ...
  for (let row = 0; row < 3; row++) {
    ...
    if (payLine) {
      results.push([payLine[0], [row * 5 + payLine[1], row * 5 + payLine[2], row * 5 + payLine[3]]]);
    }
  }
  ...
  if (diagonalPay1) {
    results.push([diagonalPay1[0], [0, 6, 12, 18, 24][diagonalPay1[1]]]);
  }
  if (diagonalPay2) {
    results.push([diagonalPay2[0], [4, 8, 12, 16, 20][diagonalPay2[1]]]);
  }
  
  return results;
}

function checkPayLine(line: number[], paySymbols: number[], nonPaySymbols: number[]) {
